...
const sheetVariants = cva(
  "glass fixed z-50 flex flex-col gap-4 p-6 shadow-2xl shadow-black/50 transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
  {
    variants: {
      side: {
        right:
  ...
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <DialogPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <DialogPrimitive.Close className="absolute right-5 top-5 rounded-full p-1.5 text-muted-foreground opacity-70 transition-opacity hover:bg-secondary/60 hover:opacity-100 focus:outline-none">
          <X className="h-4 w-4" />
